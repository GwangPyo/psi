...
export default function structuredWritingExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			functions: Type.Array(Type.Object({
				name: Type.String(),
	...
	pi.registerTool({
		...
		execute: async (_id, params, _sig, _update, ctx) => {
			...
			if (func.status !== "spec_written") throw new Error(`Function ${params.functionName} cannot be delegated from status ${func.status}.`);

			func.status = "delegated";

			const subagentModelRef = SettingsManager.create(ctx.cwd, getAgentDir(), { projectTrusted: ctx.isProjectTrusted ? ctx.isProjectTrusted() : true }).getSubagentDefaultModel();
			...
			try {
				...
				return {
					content: [{ type: "text", text: `Subagent implementation for ${params.functionName}:\n\n${result}\n\nReview this code and call \`sw_review\` to accept or reject it.\n\n${swManager.getStateSummary()}` }],
					details: {}
				};
			} finally {
				subagent.dispose();
	...
	pi.registerTool({
		...
		execute: async (_id, params, _sig, _update, ctx) => {
			...
			if (func.status !== "pending_review") throw new Error(`Function ${params.functionName} is not pending review.`);

			if (params.decision === "accept") {
				func.status = "accepted";
				return { 
					content: [{ type: "text", text: `Code ACCEPTED. Now integrate it into the file using your edit/write tools.\n\n${swManager.getStateSummary()}` }],
					details: {}
				};
			} else {
				func.status = "rejected";
				return { 
					content: [{ type: "text", text: `Code REJECTED (One-strike out). You MUST now implement ${params.functionName} yourself, and then call \`sw_mark_implemented\`.\n\n${swManager.getStateSummary()}` }],
					details: {}
				};
			}
		}
	...
	pi.registerTool({
		...
		execute: async (_id, params, _sig, _update, ctx) => {
			...
			if (func.flag === "easy" && func.status !== "rejected") {
				throw new Error(`Function ${params.functionName} is [EASY] and has not been rejected. You must delegate it.`);
			}

			func.status = "implemented";
			return { 
				content: [{ type: "text", text: `Function ${params.functionName} marked as implemented.\n\n${swManager.getStateSummary()}` }],
				details: {}
			};
		}
	});
	...
	pi.registerTool({
		...
		execute: async (_id, _params, _sig, _update, ctx) => {
			...
			const pending = Object.values(swManager.currentSubgoal.functions).filter(f => f.status !== "accepted" && f.status !== "implemented");
			if (pending.length > 0) {
			...
			return { 
				content: [{ type: "text", text: `Subgoal finished successfully! You can declare a new subgoal.\n\n${swManager.getStateSummary()}` }],
				details: {}
			};
		}
	});
